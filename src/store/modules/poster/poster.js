...
import {Widget, CopiedWidget} from 'poster/widgetConstructor';
import {
  arrMoveTop,
  arrMoveUpper,
  arrMoveLower,
...
const actions = {
  ...
  addItem({commit, dispatch, state}, item) {
    ...
    if (widgetCountLimit) {
      const currentCount = state.posterItems.filter(
        i => i.type === item.type,
      ).length;
  ...
  updateBackgroundDragInfo({state}, dragInfo) {
    state.background.dragInfo = Object.assign(
      {},
      state.background.dragInfo,
      dragInfo,
    );
  },
  // 更新组件位置、大小等
  ...
  widgetMoveToTop({commit, state, dispatch}, item) {
    ...
    commit(
      ...
      arrMoveBottom(
        state.posterItems,
        state.posterItems.findIndex(i => i.id === item.id),
      ),
    );
  },
    ...
    if (!pageConfig || !isPlainObject(pageConfig)) {
      ...
      try {
        const backgroundItem = posterItems.splice(
          posterItems.findIndex(i => i.type === 'background'),
          1,
        )[0];
        if (backgroundItem) {
