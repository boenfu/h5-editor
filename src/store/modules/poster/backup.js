...
const actions = {
  ...
  async recover({state, rootState, dispatch}, initialBackupData) {
    ...
    const backupData =
      initialBackupData ||
      (await BackupService.getBackupData(state.lastBackup.id)).backupData;
    if (backupData) {
      ...
      if (posterItems && posterItems.length > 0) {
        posterItems = posterItems.map(item => {
          const widget = new pluginConstructorMap[item.type](item);
          widget._isBackup = true;
          return widget;
        });
      }
      if (background) {
        background = new pluginConstructorMap.background(background);
      }
  ...
  async invoker({rootState, rootGetters, dispatch}, openTip = true) {
    ...
    await BackupService.saveBackupData({
      ...coreData,
      canvasSize: rootState.poster.canvasSize,
    });
